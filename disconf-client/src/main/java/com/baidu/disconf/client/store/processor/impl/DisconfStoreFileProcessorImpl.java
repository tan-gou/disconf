...
public class DisconfStoreFileProcessorImpl implements DisconfStoreProcessor {
    ...
    public void inject2Store(String fileName, DisconfValue disconfValue) {
        ...
        Map<String, FileItemValue> keMap = disconfCenterFile.getKeyMaps();
        if (keMap.size() > 0) {
            for (String key : keMap.keySet()) {
                Object object = disconfValue.getFileData().get(key);
                if (object == null) {
                    LOGGER.error("cannot find {} to be injected. file content is: {}", key,
                ...
                // 根据类型设置值
                try {
                    Object value = keMap.get(key).getFieldValueByType(object);
                    keMap.get(key).setValue(value);
                } catch (Exception e) {
